...
`--exclude-schema=des_phase_a_qualification` 排除完整私有资格 schema（包括表、触发器和
guard functions），SECRETS 包含本机基础设施 secret。
...
它会恢复 `alembic_version=20260802_0020` 却不恢复私有 schema；因此 restore/bootstrap/start
当前仍必须 `BLOCKED`，直至受保护 bootstrap 创建新空 ledger 和 issuance epoch，并拒绝旧 authority。
该候选尚未经过真实
