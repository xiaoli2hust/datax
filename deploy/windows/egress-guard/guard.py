...
LEASE_CREATION_CAPABILITY_PATH = Path(
    "/run/secrets/egress_lease_creation_capability"
)
LEASE_CREATION_CAPABILITY_HEADER = "X-DataX-Egress-Lease-Capability"
...
def read_lease_creation_capability(
    path: Path = LEASE_CREATION_CAPABILITY_PATH,
) -> bytes:
    """Load the fixed control capability without accepting a replacement value.

    The capability is a Docker secret generated by the Windows Launcher as
    32 CSPRNG bytes encoded as 64 lowercase ASCII hex characters.  The guard
    must refuse to start if that exact mounted file is absent or malformed;
    otherwise an ordinary process in the shared API/Worker network namespace
    could create a lease without controller authorization.
    """

    try:
        metadata = path.lstat()
    except OSError as exc:
        raise GuardError("LEASE_CREATION_CAPABILITY_UNAVAILABLE") from exc
    if (
        not stat.S_ISREG(metadata.st_mode)
        or path.is_symlink()
        or metadata.st_size != 64
    ):
        raise GuardError("LEASE_CREATION_CAPABILITY_INVALID")
    try:
    ...
    try:
        try:
            ...
            raise GuardError("LEASE_CREATION_CAPABILITY_INVALID") from exc
        if len(raw) != 64 or PASSWORD_PATTERN.fullmatch(value) is None:
            raise GuardError("LEASE_CREATION_CAPABILITY_INVALID")
        return bytes(raw)
    finally:
        raw[:] = b"\x00" * len(raw)
...
class AttestationHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        if self.path != "/v1/leases":
            ...
            return
        try:
            self._require_lease_creation_capability()
    ...
    def _require_lease_creation_capability(self) -> None:
        """Authenticate only the control-plane lease creation request.

        This runs before body parsing or controller access.  It protects
        against ordinary untrusted callers that share the API/Worker loopback
        network namespace; Docker secret mounts remain outside the scope of a
        hostile same-UID process boundary.
        """

        values = self.headers.get_all(LEASE_CREATION_CAPABILITY_HEADER, failobj=[])
        if len(values) != 1 or PASSWORD_PATTERN.fullmatch(values[0]) is None:
            raise GuardError("LEASE_CREATION_AUTH_INVALID")
        candidate = values[0].encode("ascii")
        if not hmac.compare_digest(self.server.lease_creation_capability, candidate):
            raise GuardError("LEASE_CREATION_AUTH_INVALID")
    ...
    def _send_guard_error(self, error: GuardError) -> None:
        ...
        if code in {"LEASE_REQUEST_INVALID"}:
            status = 400
        elif code in {"LEASE_AUTH_INVALID", "LEASE_CREATION_AUTH_INVALID"}:
            status = 401
...
class AttestationServer(http.server.ThreadingHTTPServer):
    ...
    def __init__(
        ...
        lease_creation_capability: bytes,
    ) -> None:
        try:
            canonical_capability = lease_creation_capability.decode("ascii")
        except UnicodeDecodeError as exc:
            raise ValueError("lease creation capability must be lowercase ASCII hex") from exc
        if (
            len(lease_creation_capability) != 64
            or PASSWORD_PATTERN.fullmatch(canonical_capability) is None
        ):
            raise ValueError("lease creation capability must contain exactly 64 bytes")
        self.attestation_state = state
        self.guard_controller = controller
        self.lease_creation_capability = lease_creation_capability
...
def run() -> None:
    lease_creation_capability = read_lease_creation_capability()
