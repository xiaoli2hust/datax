...
DataX Enterprise Studio V1 是面向企业内部数据工程团队的安全一次性离线全量复制控制平台。它将固定版本的 Apache DataX 封装为一条可视、可控、可核验、可审计的工作链路：

`登录 → 选择项目 → 使用已授权数据源 → 创建复制任务草稿 → 校验 → 发布不可变版本 → 准备静默源表和空目标表 → 手动执行 → 独立核验 → 审计追踪`

V1 不是通用 ETL、可重复同步、实时集成、工作流编排或 AI 平台，也不以“支持 DataX 的全部插件”为目标。V1 只承诺经过产品认证的 MySQL 8 和 PostgreSQL 15 Reader/Writer、单张静默源表到单张预创建空目标表的一次性复制，以及单节点部署。DataX 退出码 `0`、进程终态或自报写入数都不能单独证明复制成功。

### 1.1 产品定位

- **用户价值**：让不希望直接编写和保管 DataX JSON 的团队，通过受控 UI 完成可解释的一次性全量复制。
- **治理价值**：把项目边界、权限、凭据、任务版本、每次执行和审计事件纳入统一事实链。
- **运维价值**：把进程状态、数据影响、独立核验、脱敏日志、失败原因和恢复门禁集中呈现，避免依赖服务器终端。
...
在进入全量实现前，必须完成预实施 Discovery Gate：访谈 8–12 名目标用户，收集至少 20 个脱敏真实任务样本，并由 3 个设计伙伴合计完成不少于 10 个真实一次性复制任务的可用性验证。原始访谈记录、样本筛选规则、失败案例与反证必须留存；未完成时横向产品开发标记 `BLOCKED`，不得编造研究结论或用内部演示代替。

Discovery Gate 可与有界技术 spike 并行。spike 只用于证伪固定 DataX Runtime、独立 oracle、目标安全预检、网络出口或 Worker 围栏等高风险技术假设，不得扩展为完整管理后台、全量页面或横向业务模块，其产物和内部演示也不计入 V1 功能完成。
...
11. 单节点 Docker Compose 部署；Vue 3 前端、FastAPI 模块化单体、独立 Worker、PostgreSQL 和固定 DataX Runtime。V1 不依赖 Redis。
...
4. Admin 创建 EndpointPolicy 与数据源并进行真实连接测试，按成员授予 `SOURCE_USE` / `TARGET_USE`。
5. Admin 为获准的源修订 → 目标修订创建 TransferPolicy；`SENSITIVE` 策略由另一个 Admin 完成第二次批准。
6. 平台保存连接元数据和 AEAD 密文凭据，返回内容始终脱敏；运行网络访问必须被 EndpointPolicy 的 DNS 与出口规则强制约束。
7. 用户进入项目后只能看到该项目及其有权访问的资源。
...
| 需求 ID | 用户故事 | 验收摘要 |
|---|---|---|
| PRD-FR-DISC-001 | 作为产品负责人，我要用真实用户和任务证据验证 V1 的问题、边界和恢复流程，再批准横向开发。 | 完成 8–12 名目标用户访谈、至少 20 个脱敏真实任务样本，以及 3 个设计伙伴合计不少于 10 个真实一次性复制任务；证据包含失败案例、反证、样本规则、结论和负责人签署。未通过时除有界技术 spike 外，Phase 2—8 均保持 `BLOCKED`。 |
...
| RSK-05 | Worker 或主机中断造成运行态和目标影响不确定 | 错误状态、孤儿进程或重复写入 | 进程树 fencing、Worker 心跳、启动对账、`LOST`、保守 `data_effect` 和恢复门禁 |
...
- Discovery Gate 已完成：8–12 名目标用户、至少 20 个真实任务样本、3 个设计伙伴合计不少于 10 个真实复制任务；证据可追溯且包含反证。未完成时仅允许有界技术 spike，横向产品开发保持 `BLOCKED`。
