...
CREATEROLE 或继承 owner 的能力。当前 head `20260802_0017` 还将 Worker 角色上限固定为
12（`0016`），并为该角色设置 30 秒 `idle_session_timeout` 与 15 秒
`idle_in_transaction_session_timeout`（`0017`）。正常退出显式释放共享 pool；异常终止遗留的
...
执行状态；发布前若要求数据库层强制该边界，必须补充按表授权、受约束函数/触发器及
PostgreSQL 集成证据。
