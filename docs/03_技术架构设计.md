...
> loopback attestation 与 exact selected-IP 短租约候选实现，但尚未完成后端所有连接阶段
...
flowchart LR
    ...
    Proxy -->|Compose control 网络 /api/v1| Guard["egress-guard<br/>共享 Linux netns + nftables"]
...
| `launcher.exe` | Windows x64 原生本地生命周期控制器 | 单实例、依赖/端口/磁盘前置检查、生成并校验安装实例标识与数据卷标签、本机 PostgreSQL/JWT/HMAC secret、独立凭据 KEK 及 ACL、启动 Compose、调用固定迁移/生命周期/首次管理员/备份/诊断 helper、轮询就绪、打开默认浏览器、安全停止 | 读取或解密业务数据源凭据、直接连接业务数据库、代替 API/Worker 推进业务状态、把 secret 放入命令行 |
| `web` | Vue 3、TypeScript、Element Plus、反向代理 | 页面、静态资源、同源 API 转发；唯一映射 `127.0.0.1:17860` | 绑定 `0.0.0.0`/局域网地址、保存 refresh token、连接数据源 |
| `egress-guard` | PostgreSQL `15.18-alpine3.24` 基础镜像 + Python stdlib + nftables + PostgreSQL client | 与 API/Worker 共享 Linux netns；以只读 ACTIVE policy view 审批 selected-IP 租约；原子维护默认拒绝规则并提供 loopback attestation | 保存业务状态、读取数据源凭据、接受宿主/LAN 请求、把 policy CIDR 整段写为外部 allow |
...
| `worker` | Python 3.12.13 `slim-trixie`（Debian 13）+ JDK 8 + DataX `datax_v202309` | 公平领取两类事实、reconciler、预检/恢复探针、临时凭证、DataX 进程树、LogGap、核验、心跳和汇总 | 暴露公网 API、接受任意命令 |
| Docker named volumes | `des-postgres-data`、`des-log-data`、`des-workspace-data` | 保存 PostgreSQL、脱敏日志和不含秘密的有界 oracle 工作区；三卷必须同时存在并携带当前安装实例和各自 role 标签；由 Docker Desktop 管理，升级/重启不丢失 | 保存 Job JSON、未脱敏 Runtime 日志或 DataX 进程工作目录；直接映射任意用户目录；卷部分丢失后自动创建空卷 |
...
当前数据库迁移 `20260731_0013` 已把迁移 owner、`datax_api` 和 `datax_worker` 分成
独立登录和独立密码；owner secret 不挂载给 API/Worker，两个运行角色均无 DDL、TEMP、
CREATEDB、CREATEROLE 或继承 owner 的能力。当前两类运行角色仍被授予相同的全表
`SELECT/INSERT/UPDATE/DELETE` 与 sequence 使用权，细粒度的状态写入职责主要由应用层
执行。不得把“独立登录/no-DDL”描述成数据库已经阻止 API 更新执行状态；发布前若要求
数据库层强制该边界，必须补充按表授权、受约束函数/触发器及 PostgreSQL 集成证据。
...
3. 启动前必须检查 Windows 11 x64、WSL2/Docker Engine/Compose 版本、Docker Linux
   ...
   全新初始化必须以 ACL 受控的 `initialization-incomplete` 日志为恢复依据，先完整生成
   和校验 secret，再创建/核验卷，最后提交 `installation-id`；只有在未发现任何产品或
   数据卷容器、既有卷标签匹配且卷出现时 secret 已完整时才可补齐。不得自动删除卷，
   也不得把缺 secret 的既有卷当成全新安装。
4. 前置检查通过后，launcher 以固定 project name 执行 `docker compose up -d`，轮询
...
- 本地系统备份由 launcher 在安全停写后调用固定 Worker helper，原子生成两个分离的
  加密包：`.dxdata` 只包含 custom-format PostgreSQL 逻辑 dump、脱敏日志和三个固定发布
  元数据；`.dxkeys` 单独包含四类数据库角色密码、JWT/HMAC 与受支持 KEK。两包使用不同
...
ADR-0011 已接受下列未来架构，但当前没有相应 reader、schema、Compose override 或运行
实现，因此正式服务仍为 deny-all：

1. 构建先固定不可变发布 payload P，P 绑定 API/Worker/Web/egress/PostgreSQL 五镜像、
   Worker Runtime、插件 JAR、依赖/许可证、SBOM、commit 和 payload root；QH/QR、最终
   manifest 与最终安装包不进入 P。
2. 只有受保护 Windows qualification harness 可读取短期、一次性、签名的 QH。QH 仅允许
   精确 P 完成真实 E3/私有 Windows harness/payload qualification（不是 E4），不把任何测试
   或 harness 资格反映为普通用户 plugin 状态，也不允许 UI/API 配置或环境变量自行打开。
3. 独立 Release Qualification Authority 复核 Phase A 证据后，为同一 P 签发 detached QR。
   未来 API/Worker 的受信 reader 必须用 P 内固定公钥、签名、purpose、有效期以及
   commit/image/runtime/JAR/依赖/许可证逐项校验 QR；缺失或任何错配时四个执行检查点
   都保持 deny-all。
4. QR 只作为私有最终候选的普通模式资格。最终 Setup/Launcher/release manifest 在 QR
   之后生成并绑定 QR 文件哈希，却不得被 QR 反向绑定；随后在无 QH 的标准 Compose 上对
   精确最终包执行 Phase B Windows E4，才可派生 `WINDOWS_E4_CERTIFIED`。该状态不等于公开
   分发，只有 ADR-0010 的同一 F hosted 候选根证明、完整发布 validator 和有效 PR 都通过，
   才可公开分发。
...
| `launcher.exe` → Docker Desktop/WSL2 | Compose/镜像篡改、命令注入、secret 出现在参数或诊断中 | 安装包签名、固定绝对路径/参数数组/project name、清单与镜像摘要校验、Windows named mutex、ACL 受限配置与 Docker secret、诊断脱敏 |
...
- launcher 前置检查：在 Compose 启动前检查 Windows/WSL2/Docker/Compose、Linux
  container 模式、虚拟化、端口、ACL、镜像摘要、named volume、内存与磁盘；它不是
  HTTP API，也不能被容器健康状态替代。
...
- `http://127.0.0.1:17860/api/v1/health/ready`：管理平面关键门检查 PostgreSQL、数据库
  迁移、API 日志卷与追加式审计哈希链；能力状态另列最近 Worker 心跳、本次 Linux
...
17. 默认 200 GiB 主机同时满足存储公式、水位 admission、轻量/大表组合档和队列 SLO；
    超水位时 fail closed。
18. `Setup.exe` 在 Windows 11 x64 完成安装/卸载，launcher 单实例、前置检查、Compose
    启动、ready 轮询、首次 Admin GUI 引导、打开浏览器和安全停止均有真实 Windows 验收；
    临时密码只经标准输入且不留痕；缺失 Docker/WSL2、端口冲突或摘要错误不能显示成功。
