...
>
> 迁移 `20260802_0020` 与同轮源码新增的 P/QH 解析、私有 payload/runtime/job
> binding 以及 Phase-A nonce/grant 持久化，只是 ADR-0011 的 E1 基础件。它既没有
> 专用受保护 ledger role/function，也没有 protected issuer/consumer、私有
> API/Worker/Compose override、QH source、受保护 harness、QR reader 或任何真实 E3/E4
> 证据；标准产品路径继续 deny-all。
>
> 架构版本：1.2
>
> 适用产品版本：V1
...
CREATEROLE 或继承 owner 的能力。当前 head `20260802_0020` 保留 Worker 角色上限
...
`phase_a_qualification_grants`，并撤销 `PUBLIC`、`datax_api`、`datax_worker` 的 schema/table/
guard-function 直接权限。guard functions 固定 `search_path=pg_catalog`，跨表查询使用完整
schema 名，避免未来 issuer 被调用方 search path 影子化。这一窄 E1 基础不等于已实现专用
issuer role/function；当前标准 API/Worker 既没有读取/写入路径，也不能据此运行 qualification
或解除普通执行 deny-all。
...
- `des_phase_a_qualification` 不属于可恢复的普通产品事实。自 `20260802_0020` 起，标准系统
  ...
  nonce hash、P/harness/plugin binding、grant lifecycle、触发器和 guard functions）；不得因
...
- 标准 restore 不得恢复、复制或重新激活任何 QH/PAG authority。因为 dump 仍会恢复
  `alembic_version=20260802_0020` 却不含私有 schema，当前真实 `pg_restore`、应用启动与
...
binding；`datax_api`、`datax_worker` 对两张 Phase-A 表的直接权限已被撤销。它仍不是一个
可运行的 qualification 通道：没有专用受保护 ledger role/function、protected issuer/consumer、
private API/Worker/Compose override、QH source、受保护 harness、QR reader 或真实 E3/E4。
普通 API/UI、Settings、环境变量、标准 Compose 和公开 Plugin Manifest 没有 QH/PAG 入口，
生产服务继续 deny-all。
...
25. 在任何 Phase-A E3 运行之前，受保护部署必须证明 QH 只由固定 P/harness 的私有
    ...
    当前 0020 迁移只验证账本基础约束，缺少专用 ledger role/function、issuer/consumer 和
    私有运行路径，故本项仍为 `NOT_RUN/BLOCKED`，不得称为 E3 或 E4。
