...
>
> 迁移 `20260802_0020/0021/0022` 与同轮源码新增的 P/QH 解析、私有 payload/runtime/job
...
CREATEROLE 或继承 owner 的能力。当前 head `20260802_0022` 保留 Worker 角色上限
...
pytest `29 passed`）；它不扩展为一般运行角色职责隔离，更不是 E3/E4。
...
该边界已在本轮真实 PostgreSQL 15 E2 由 runtime-role direct-DB 负向验证（脚本退出 `0`，pytest `29 passed`）。
...
当前没有私有 Phase-A Worker/runner，`PHASE_A_HARNESS` 在标准部署内保持硬禁用。它不能由 Settings、
环境变量、标准 Compose、Launcher 或普通维护接口打开。未来受保护 runner 启用前，必须先落地 ADR-0011 §3.1 的
private execution lock/fence、源静默/目标独占 confirmation、受保护 audit checkpoint，以及独立凭据、日志、维护和
...
- `des_phase_a_qualification` 不属于可恢复的普通产品事实。自 `20260802_0020` 起，标准系统
  ...
  guard 函数和私有 0021/0022 `SECURITY DEFINER` issuer/consumer entrypoints。不得因 PostgreSQL
...
- 标准 restore 不得恢复、复制或重新激活任何 QH/PAG/PEA authority。因为 dump 仍会恢复
  `alembic_version=20260802_0022` 却不含私有 schema、其 private issuer/consumer entrypoints 或无登录 ledger roles（而
...
25. 在任何 Phase-A E3 运行之前，受保护部署必须证明 QH 只由固定 P/harness 的私有
    ...
    当前 0020/0021/0022 迁移已实现账本基础约束、PEA issuer-authorize/consumer-read 边界和普通
    API/Worker/Recovery/日志的 `STANDARD`-only 限制；本轮临时真实 PG15 的
    `scripts/test-postgres-e2.sh` 退出 `0`、PostgreSQL pytest `29 passed`，已覆盖 0022
    authorization/RLS/普通路径边界、私有 schema exclusion 与空库 `pg_restore` 探针。仍缺少
