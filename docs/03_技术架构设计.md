...
3. 启动前必须检查 Windows 11 x64、WSL2/Docker Engine/Compose 版本、Docker Linux
   ...
   把“脚本只输出容量”描述为内核已禁止读取卷内容。任一 Docker/协议异常或任一卷当前可用空间
   低于 40 GiB 都失败关闭。该数值是启动水位，不是每卷预留；三卷通常共享同一个 Docker
   文件系统，因此并不要求 `3 × 40 GiB`。200 GiB 只用于大表/30 天日志组合验收的推荐容量。
   探针用 `--pull=never`，错误/超时/输出无效时只按重新认证的 immutable ID 删除
...
- 数据源凭据 KEK 必须由 OS CSPRNG 独立生成，不能与 refresh-token 或 idempotency HMAC
  key 复用；API 与 Worker 只读挂载同一版本化 KEK 文件。全新数据库另有 migration 后的
  一次性 `keyring-bootstrap`，仅以 `datax_api`、API 数据库密码、同一 KEK 和 idempotency
  HMAC 幂等登记/核验指纹后退出；它没有 JWT/refresh、owner、lease secret、端口或数据卷，且
  与 API/Worker 共享 `egress-guard` 网络命名空间，不能旁路默认拒绝出口。
  Worker 仍仅做只读核验，`postgres`、`egress-guard` 与 `web` 不得获得 KEK。
...
启动准入与工作负载容量必须分开。Launcher 的最低启动条件是：`%LOCALAPPDATA%` 所在卷
和实际 Docker 数据文件系统在各自检查时均有至少 40 GiB 当前可用空间；Docker 的三个
产品卷通常共用同一文件系统，这不是三份 40 GiB 预留，也不等于 120 GiB。低于该水位，
Launcher 在启动 Compose 前失败关闭。

以下 200 GiB 是**大表复制/30 天日志保留的推荐验收包络**，不是安装或普通本地启动门槛。
要宣称该档的吞吐与保留能力，Windows 工作站的 Docker Desktop 虚拟磁盘所在卷必须提供
至少 200 GiB 可用 SSD，并按以下 named-volume/镜像预算验收：
...
16. 未领取取消由 reconciler 在 SLO 内收敛；重启后持久公平游标不重置，容量包络内跨
    项目任务不会被单一项目永久饥饿。
17. 最低启动水位（实际 Docker 数据文件系统 40 GiB）与 200 GiB 大表推荐包络分别经
    对应的存储公式、水位 admission、组合档和队列 SLO 验证；超水位时 fail closed。
