...
>
> 迁移 `20260802_0020/0021/0022/0023` 与同轮源码新增的 P/QH 解析、私有 payload/runtime/job
> binding、Phase-A nonce/grant 持久化及 PEA 私有数据库授权边界，只是 ADR-0011 的 E1 基础件。
...
> 与 recovery/reconciler 只处理 `STANDARD` Execution；它没有 private Execution 创建/rerun、credential provisioning、
> 私有 API/Worker/Compose override、四检查点、QH source、受保护 harness、QR reader 或任何真实
> E3/E4 证据。0023 只补入复用标准全局目标锁和 fencing epoch 的未接线 private runner SQL 前置原语；
> 它也不会创建 runner、启动 DataX 或解除标准产品路径的 deny-all。
>
> 架构版本：1.2
>
> 适用产品版本：V1
...
CREATEROLE 或继承 owner 的能力。当前 head `20260802_0023` 保留 Worker 角色上限
...
pytest `30 passed`）；它不扩展为一般运行角色职责隔离，更不是 E3/E4。
...
该边界已在本轮真实 PostgreSQL 15 E2 由 runtime-role direct-DB 负向验证（脚本退出 `0`，pytest `30 passed`）。
这一窄 E1 实现加数据库 E2 证据不等于已实现 protected harness、私有创建/重跑入口、
...
- `des_phase_a_qualification` 不属于可恢复的普通产品事实。自 `20260802_0020` 起，标准系统
  备份和诊断包必须以固定 `--exclude-schema=des_phase_a_qualification` 排除完整私有 schema：三张表、
  nonce hash、P/harness/plugin binding、grant lifecycle、PEA binding、schema 内触发器、私有 ledger
  guard 函数和私有 0021/0022/0023 `SECURITY DEFINER` issuer/consumer/lock-fence entrypoints。不得因 PostgreSQL
...
- 标准 restore 不得恢复、复制或重新激活任何 QH/PAG/PEA authority。因为 dump 仍会恢复
  `alembic_version=20260802_0023` 却不含私有 schema、其 private issuer/consumer/runner entrypoints 或无登录 ledger roles（而
...
2. HQA 只可在受保护环境为精确 P 与固定 harness 签发短期、一次性 QH。受保护私有
   ...
   consumer read 只在当前事实仍相等时返回它。私有创建、rerun、Worker claim、Worker start 四检查点
   仍未接线，因此这个数据库 record 不能启动 DataX。QH/PAG/PEA 只允许真实 E3/私有 Windows harness/payload qualification，
...
Execution 创建/`rerun` 入口。J0b.1 的 PEA JSON Schema 描述 0022 的 private read record；读取
binding 不会创建 Execution 或启动 DataX，也不能改变生产 deny-all。private API/Worker 四检查点、
...
25. 在任何 Phase-A E3 运行之前，受保护部署必须证明 QH 只由固定 P/harness 的私有
    ...
    当前 0020/0021/0022/0023 迁移已实现账本基础约束、PEA issuer-authorize/consumer-read、未接线 global lock/fence 边界和普通
    API/Worker/Recovery/日志的 `STANDARD`-only 限制；本轮临时真实 PG15 的
    `scripts/test-postgres-e2.sh` 退出 `0`、PostgreSQL pytest `30 passed`，已覆盖 0022
    authorization/RLS/普通路径、0023 global lock/fence、私有 schema exclusion 与空库 `pg_restore` 探针。仍缺少
    protected QH/PAG/PEA source/override、私有 Execution 创建/重跑、私有 API/Worker
