...
>
> 迁移 `20260802_0020/0021` 与同轮源码新增的 P/QH 解析、私有 payload/runtime/job
> binding 以及 Phase-A nonce/grant 持久化，只是 ADR-0011 的 E1 基础件。0021 新增无登录
> ledger owner / issuer / consumer、最小 `SECURITY DEFINER` 函数和 private adapter；它仍没有
> protected harness credential provisioning、私有 API/Worker/Compose override、Execution authorization、
> QH source、受保护 harness、QR reader 或任何真实 E3/E4 证据；标准产品路径继续 deny-all。
>
> 架构版本：1.2
>
> 适用产品版本：V1
...
CREATEROLE 或继承 owner 的能力。当前 head `20260802_0021` 保留 Worker 角色上限
...
的状态写入职责主要由应用层执行。不得把“独立登录/no-DDL”描述成数据库已经阻止 API 更新
执行状态；ADR-0013 与 `contracts/runtime-db-write-matrix.v1.json` 已确定按表/列授权、
受约束函数/触发器及 PostgreSQL 集成证据的目标，但迁移尚未实施，当前权限事实没有改变。
...
若预先存在，迁移失败关闭；ledger owner 后续新建函数默认撤销 `PUBLIC EXECUTE`。这一窄 E1 基础不等于
已实现 protected harness、Execution authorization 或私有运行通道：标准 API/Worker 没有角色凭据、
读取/写入接线，不能据此运行 qualification 或解除普通执行 deny-all。
...
3. 启动前必须检查 Windows 11 x64、WSL2/Docker Engine/Compose 版本、Docker Linux
   ...
   当前 LEGACY 初始化必须以 ACL 受控的 `initialization-incomplete` 日志为恢复依据，先完整
   生成和校验 secret，再创建/核验卷，最后提交 `installation-id` 与不可覆盖的 LEGACY
   指针；只有在未发现任何产品或数据卷容器、既有卷标签匹配且卷出现时 secret 已完整时才可
   补齐。FRESH 的带代际 pending journal 初始化尚未实现，不能由现有 root journal 猜测或
   拼接。不得自动删除卷，也不得把缺 secret 的既有卷当成全新安装。
...
- `des_phase_a_qualification` 不属于可恢复的普通产品事实。自 `20260802_0020` 起，标准系统
  备份和诊断包必须以固定 `--exclude-schema=des_phase_a_qualification` 排除完整 schema（两表、
  nonce hash、P/harness/plugin binding、grant lifecycle、触发器、guard functions 和 0021
  `SECURITY DEFINER` entrypoints）；不得因
  PostgreSQL 全库逻辑 dump、诊断 SQL 或恢复 staging 间接携带它们。候选 launcher 的 schema
  exclusion、TOC 与临时库 restore probe 最多构成 E2，不是完整 restore、DataX E3 或 Windows E4
  证据。
- 标准 restore 不得恢复、复制或重新激活任何 QH/PAG authority。因为 dump 仍会恢复
  `alembic_version=20260802_0021` 却不含私有 schema、entrypoints 或无登录 ledger roles，当前真实 `pg_restore`、应用启动与
...
ADR-0011 的资格链严格是 `P → QH → PAG → QR → F → PR`，各对象的结论不能互换：
...
2. HQA 只可在受保护环境为精确 P 与固定 harness 签发短期、一次性 QH。受保护私有
   ...
   Reader/Writer pair 的 PAG。QH/PAG 只允许真实 E3/私有 Windows harness/payload
   qualification，明确不是 E4、Plugin Manifest 状态或普通用户许可。
...
4. QR 只作为私有最终候选的正常模式资格。最终 Setup/Launcher/release manifest 在 QR
   之后生成并绑定 QR 文件哈希，却不得被 QR 反向绑定；随后在无 QH/PAG 的标准 Compose 上
...
当前只落地 E1 基础件：P/QH/PAG Schema、失败关闭 P/QH parser、私有 payload/runtime/
job binding primitives，以及 PostgreSQL nonce/grant 账本。账本只保留 nonce 哈希和不可变
binding；`20260802_0021` 将私有 schema、两张表和 guard functions 的 owner 交给无登录的
ledger owner，并仅向无登录 issuer/consumer 角色精确授权三个 `SECURITY DEFINER` 函数：原子
nonce+PAG 签发、撤回与 current-grant 读取。标准 API/Worker/egress 以及 issuer/consumer 都没有
直接表 DML 权限；private adapter 只能接收未来受保护 harness 显式注入的 Engine。

它仍不是一个可运行的 qualification 通道：标准 API/UI、Settings、环境变量、Compose、Launcher 和
公开 Plugin Manifest 没有角色凭据、QH/PAG source/override 或 adapter 接线。J0a 的 grant 没有
`execution_id`，读取 binding 不会创建 Execution 或启动 DataX；它不能改变生产 deny-all。
private API/Worker 四检查点、受保护 harness、QR reader 和真实 E3/E4 仍未实现。
...
25. 在任何 Phase-A E3 运行之前，受保护部署必须证明 QH 只由固定 P/harness 的私有
    source/override 使用，PAG nonce 原子消费且 binding 不可变，并且标准 Compose/API/Worker
    无 QH/PAG 入口；之后还须真实 MySQL/PostgreSQL、独立 oracle 与受保护 harness 证据。
    当前 0020/0021 迁移已实现账本基础约束与专用 PostgreSQL role/function 边界；临时真实 PG15
    `22 passed` 已覆盖预存私有角色失败关闭和 future-function `PUBLIC EXECUTE` 默认权负例，但仍缺少
    protected QH/PAG source/override、私有 API/Worker 四检查点、Execution authorization、
    受保护 harness 与真实外部链路，故本项仍为 `NOT_RUN/BLOCKED`，不得称为 E3 或 E4。
