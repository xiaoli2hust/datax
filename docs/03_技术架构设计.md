...
CREATEROLE 或继承 owner 的能力。当前 head `20260802_0018` 还将 Worker 角色上限固定为
...
6. 数据源创建/更新的连接探针、连接测试、Schema 探测和任务校验由 API 的 Credential
   Service 编排受限 connector；它们不是 Worker 的 DataX 执行路径。外部数据库账号必须是
   对该操作所需的最小权限账号；Schema/metadata 读取必须只读，并设置连接、查询和单次
   操作总时限。
7. **目标边界（ADR-0014；当前候选尚未实现，`ASR-013=OPEN`）**：上述五个入口必须采用
   A（短事务授权/不可变绑定快照）→ B（无产品 DB 锁的 DNS/egress/解密/connector）→ C
   （短事务复核/写 evidence 或响应）三阶段。不得在持有 Organization/Project/Datasource/Job
   锁时进行 DNS、egress、probe 或 Schema I/O；配置、密钥、授权或任务 scope 漂移时必须拒绝
   旧结果，不能把连接测试称为已限速。准入、总 deadline、409 stale 和 429 admission 的具体
   合同以 ADR-0014 及后续 OpenAPI/实现为准。
