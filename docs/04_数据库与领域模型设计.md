...
> `DatasourceUsageGrant.row_version`，以及 `20260802_0020` 建立、由 `20260802_0021` 加固角色与
> 函数边界的私有 Phase-A qualification nonce/grant 账本；所有应用审计 writer 在同一事务推进链头，
...
触发器、guard functions 或 0021 `SECURITY DEFINER` entrypoints 复制到恢复目标。候选 launcher 的 schema exclusion、TOC 与临时库
restore probe 至多是 E2，不是完整 restore、DataX E3 或 Windows E4。完整 restore 当前仍
`BLOCKED`：恢复库会带 `alembic_version=20260802_0021` 而没有私有 schema。未来 restore bootstrap 必须
...
> **状态：E1 持久化和 PostgreSQL 角色/函数基础件；已取得受限 PostgreSQL E2，非普通产品能力。** 这两张 PostgreSQL
> 表位于 closed-default `des_phase_a_qualification` schema，由 `20260802_0020` 创建；
> `20260802_0021` 将对象 ownership 转给无登录 ledger owner，并创建只供未来私有 harness 使用的
> issuer/consumer `SECURITY DEFINER` entrypoints。它们服务于 ADR-0011 未来
...
有效时读取它，并在过期时先把它转为 `EXPIRED`。它刻意没有 `execution_id`：当前
`PhaseAExecutionBinding` 只是在进程内或由 durable reader 重建的 binding；未来 J0b 必须另行设计
持久、不可变且覆盖私有 API/Worker 四检查点的 Execution authorization，不能用可变的事后关联
绕过 binding。
...
只有专用无登录 issuer 可执行签发/撤回函数，专用无登录 consumer 可执行读取函数；两个函数集合
均不能互相替代。future private adapter 只能使用由受保护 harness 显式注入的专用 Engine，并不得
接受请求体、Settings 或环境变量的 grant/role/DSN。它仍不是 protected issuer 的完整实现：没有
私有 API/Worker/Compose override、QH source、受保护 harness、Execution authorization 或真实跨进程
运行路径；migration owner/数据库管理员也不是 HQA/RQA 信任根。标准启动与普通产品路径继续
production deny-all。
...
dump 会保留 `alembic_version=20260802_0021` 却排除该 schema。未来 protected restore bootstrap 才能创建
...
- `20260802_0020` 为 PostgreSQL 专用迁移，在 closed-default
  ...
  `PUBLIC EXECUTE`。标准 Compose 不配置这些角色凭据，且函数不含 `execution_id` 或 DataX 启动能力。两者的
  downgrade 只撤销 E1 foundation；它不构成 protected harness、真实 DataX E3、Windows E4 或发布
  回滚证据。
- 任何支持 `20260802_0021` 及更高版本的标准 backup/diagnostics 都必须固定排除
...
23. Phase-A 私有 ledger 必须拒绝 nonce 重放、binding 修改、grant 删除/截断及非单调生命周期；
    标准运行角色和 issuer/consumer 对两表的实际直接读写/截断必须被 PostgreSQL 拒绝；issuer 只能
    调用原子签发/撤回函数，consumer 只能读取 current grant。当前 disposable PostgreSQL 15 的
    `22 passed` 已作为受限 E2 验证该 role/function/persistence 边界与 issuer → consumer → revoke
    往返、预存私有角色失败关闭和 future-function `PUBLIC EXECUTE` 默认权负例，不证明受保护 harness、
    私有 API/Worker Execution authorization、真实 E3 或 E4。
