...
| 审计 | 追加写，不提供更新和删除业务接口；链头周期性签名并锚定到数据库外 |
...
- **Audit 聚合**：追加式 AuditEvent 哈希链、审计公钥 keyring 及数据库外签名锚点元数据。
...
此表只保存外部 KEK keyring 的非秘密注册信息，不保存 KEK 字节。
...
`DESTROYED`。服务 readiness 必须证明所有 `ACTIVE/DECRYPT_ONLY` 版本在外部只读 keyring
中可用且指纹匹配。
...
| `peer_ip` | inet | 是 | 建连后观测的远端地址，必须等于 selected_ip |
...
连接固定到 `selected_ip`，TLS 仍验证原 hostname，建连后校验 `peer_ip`。TTL 到期、策略
revision/egress 版本变化或证据不完整时不得复用证据；各连接阶段都生成自己的证据，不能
把创建数据源时的一次解析永久当成运行事实。运行类证据必须且只能关联 Execution 或
RecoveryProbe 之一，并匹配该类当前 attempt/fence；失租后不得补写“允许”证据。
...
6. source/target 列在 mappings 中分别唯一，映射顺序即 DataX column 顺序。
...
| `target_empty_evidence` | jsonb | 否 | Worker 的 revision、表指纹、COUNT、时间和证据哈希 |
| `runtime_snapshot` | jsonb | 否 | DataX/JDK/Python/插件哈希以及本次接受的目标独占声明身份/有效期快照 |
...
只有 Worker 进程及其内置 reconciler 数据库角色可以更新三组状态。API 数据库角色只
允许 INSERT 初始 `QUEUED/NONE/NOT_STARTED` Execution、INSERT CancelRequest，以及通过
受约束数据库函数把非终态 Execution 的目标声明从 `ACTIVE` 更新为 `REVOKED` 并写审计；
API 不能借撤回接口直接改三组执行状态。
...
Worker 领取时生成的 `runtime_snapshot` 必须固定
...
用了哪个 secret”。领取后 secret 转 `RETIRED` 不影响已固定工作；转
...
| `host_boot_id` | varchar(128) | 是 | 主机/容器启动标识 |
...
API 数据库角色只能 INSERT 初始 `QUEUED/NOT_STARTED` Probe；只有 Worker/reconciler 角色
可推进状态、结果、Attempt 和 gate，规则与 Execution 的数据库权限隔离一致。
...
数据库只保存 Ed25519 公钥 keyring；签名私钥位于独立 Secret/HSM，数据库写账号不可读。
...
| AuditEvent、AuditCheckpoint、审计公钥 keyring | 730 天或关联事件/保全更长者 | 到期先导出并保留可验证锚点连续性；所需公钥不得提前删除 |
...
- 数据库备份必须包含 KEK 非秘密注册信息、外部 keyring 版本清单和审计锚点引用；KEK
  字节与数据库备份分离保管。没有对应 KEK 的凭证密文不可恢复。
