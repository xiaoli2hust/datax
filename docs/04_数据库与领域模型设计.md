...
`BLOCKED`：恢复库会带 `alembic_version=20260802_0022` 而没有私有 schema。未来 restore bootstrap 必须
...
> **状态：E1 持久化和 PostgreSQL 角色/函数基础件，且本轮真实 PostgreSQL 15 E2 已覆盖 0021/0022 数据库边界（脚本退出 `0`，pytest `29 passed`）。** 前两张 PostgreSQL
...
> issuer authorize/consumer read 边界。它们服务于 ADR-0011 未来
...
可执行 current-grant 与 PEA read 函数；两个函数集合均不能互相替代。`qualification.execution_authorization`
...
dump 会保留 `alembic_version=20260802_0022` 却排除该 schema。未来 protected restore bootstrap 才能创建
...
> **状态：E1 数据库授权基础已实现；本轮真实 PostgreSQL 15 E2 已验证 0022 的授权、普通路径拒绝和 runtime-role RLS（脚本退出 `0`，pytest `29 passed`）。**
...
当前没有私有 Phase-A Worker/runner，private row 在标准运行面保持硬禁用。启用它前必须以独立纵向切片实现
private execution lock/fence、源静默与目标外部独占 confirmation、可关联 execution/fence 的受保护 audit checkpoint，
以及专用凭据、私有日志与维护路径；PEA 的 unique/read 语义或标准 `TargetCopyLock` 不能替代这些门禁。
...
- `20260802_0020` 为 PostgreSQL 专用迁移，在 closed-default
  ...
  角色凭据，且没有私有创建/rerun、Worker 四检查点或 DataX 启动调用链。三次迁移的 downgrade
  只撤销 E1 foundation；它不构成 protected harness、真实 DataX E3、Windows E4 或发布回滚证据。
- 任何支持 `20260802_0022` 及更高版本的标准 backup/diagnostics 都必须固定排除
...
23. Phase-A 私有 ledger 必须拒绝 nonce 重放、binding 修改、grant/PEA 删除或截断及非单调 grant
    ...
    `scripts/test-postgres-e2.sh` 退出 `0`、PostgreSQL pytest `29 passed`，已提供 0021/0022 role/function/
    PEA authorization、普通路径拒绝、RLS 与标准 dump→空库 `pg_restore` 探针的受限 E2；私有创建/Worker
