...
> `DatasourceUsageGrant.row_version`，以及 `20260802_0020` 建立的私有 Phase-A
> qualification nonce/grant 账本；所有应用审计 writer 在同一事务推进链头，
...
> 仅 `20260802_0020` 新增的 Phase-A ledger 是明确例外：它位于 closed-default 私有 schema
> `des_phase_a_qualification`，并撤销 `PUBLIC`、`datax_api`、`datax_worker` 的 schema/table/
> guard-function 直接权限；guard functions 固定 `search_path=pg_catalog` 并使用 schema-qualified
> lookup。尚无 protected issuer role/function 或 protected issuer/consumer，不能把该 E1 基础
> 表述为完整职责隔离或可运行的资格通道。
...
私有 schema `des_phase_a_qualification`（其中的 `phase_a_qualification_nonces` 与
`phase_a_qualification_grants`）不是可携带的应用业务状态。标准 system backup 与 diagnostics
必须以固定 `--exclude-schema=des_phase_a_qualification` 从逻辑 dump、归档、导出清单、诊断 SQL
输出和支持包中排除整个 schema，不能把 nonce hash、P/harness/plugin binding、grant 生命周期、
触发器或 guard functions 复制到恢复目标。候选 launcher 的 schema exclusion、TOC 与临时库
restore probe 至多是 E2，不是完整 restore、DataX E3 或 Windows E4。完整 restore 当前仍
`BLOCKED`：恢复库会带 `alembic_version=0020` 而没有私有 schema。未来 restore bootstrap 必须
...
- **Phase-A Qualification Ledger（私有发布辅助聚合）**：`P → QH → PAG → QR` 链中，
  仅记录已验证 QH 的 nonce 重放事实和精确 payload/harness/Reader-Writer binding。它不属于
  普通插件目录、Execution、审计或公开发布事实；E1 当前没有 `Execution` 外键或任何标准
  API/Worker 可访问路径。
...
> **状态：E1 持久化基础件，非普通产品能力。** 这两张 PostgreSQL 表位于
> closed-default `des_phase_a_qualification` schema，由
> `20260802_0020` 创建，服务于 ADR-0011 未来 `P → QH → PAG → QR` 的 Phase-A 私有
> qualification。它们不写入 `plugin_catalog`、公开 Plugin Manifest 或普通 Execution，不能
...
qualification、payload root、有效期与消费顺序一致。它刻意没有 `execution_id`：当前
`PhaseAExecutionBinding` 只是在进程内把已验证 authorization 与一个 JobVersion 逐项比对的
E1 primitive；未来必须由受保护的原子 issuer/function 设计持久且精确的 Execution linkage，
不能用可变的事后关联绕过 binding。

迁移以 closed-default schema、`REVOKE ALL ON SCHEMA/TABLE/FUNCTION` 使 `PUBLIC`、`datax_api`、
`datax_worker` 不能直接发现、读写或执行这部分 ledger；实际 runtime 登录的 SELECT/INSERT/
UPDATE/DELETE/TRUNCATE 都被 PostgreSQL 拒绝。它仍不是 protected issuer 的实现：没有专用 issuer
database role/function、私有 API/Worker/Compose override、QH source、受保护 harness 或跨进程
consumer；migration owner/数据库管理边界也不是 HQA/RQA 信任根。标准启动与普通产品路径继续
production deny-all。
...
dump 会保留 `alembic_version=0020` 却排除该 schema。未来 protected restore bootstrap 才能创建
...
- `20260802_0020` 为 PostgreSQL 专用迁移，在 closed-default
  `des_phase_a_qualification` schema 创建 Phase-A nonce/grant 账本、约束和触发器，固定
  guard-function search path 并撤销 PUBLIC/API/Worker 权限。其 downgrade 只移除该 E1
  foundation；它不构成 protected issuer、真实 DataX E3、Windows E4 或发布回滚证据。
- 任何支持 `20260802_0020` 及更高版本的标准 backup/diagnostics 都必须固定排除
...
23. Phase-A 私有 ledger 必须拒绝 nonce 重放、binding 修改、grant 删除/截断及非单调生命周期；
    `datax_api`、`datax_worker` 对私有 schema 两表的实际直接读写/截断必须被 PostgreSQL 拒绝。当前这只验证 E1
    持久化约束，不证明 protected issuer/consumer、私有 harness、真实 E3 或 E4。
