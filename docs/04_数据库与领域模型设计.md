...
> `DatasourceUsageGrant.row_version`，以及 `20260802_0020` 建立、由 `20260802_0021/0022/0023/0024` 加固角色、
> 函数、PEA、全局目标锁和 issuer-only 原子创建边界的私有 Phase-A qualification ledger；所有应用审计 writer 在同一事务推进链头，
...
`phase_a_qualification_grants`、`phase_a_execution_authorizations` 与
`phase_a_execution_create_checkpoints`）不是可携带的应用业务状态。标准 system backup 与 diagnostics
...
schema 内触发器、私有 ledger guard functions 或私有 0021/0022/0023/0024 `SECURITY DEFINER`
issuer/consumer/runner/private-create entrypoints 复制到恢复目标。`public.des_phase_a_execution_mode_guard()` 不属于该
...
`BLOCKED`：升级到当前 schema head 的恢复库会带 `alembic_version=20260802_0024` 而没有私有 schema。未来 restore bootstrap 必须
创建空 ledger、轮换并强制当前 post-restore issuance epoch，要求重新签发与重新消费资格，不能从
备份、诊断包或旧 ledger 重放 grant。

0022/0024 使 public `Execution` 与目标锁也承载 private binding，故只排除 private schema 仍不充分：任何
...
> **状态：0020--0024 已有 E1 持久化；0024 的原子创建/lifecycle 已在 2026-08-02 取得受限 PostgreSQL E2（`32 passed`）。** 前两张 PostgreSQL
...
> SQL primitive，不增加私有 lock 表或产品 runner。`20260802_0024` 新增第四张 private append-only create
> checkpoint 表和 issuer-only create→PEA→public `TargetCopyLock=RESERVED` 原子函数。它们服务于 ADR-0011 未来
...
`execution_id` 各自 `UNIQUE`，不是把 grant 改为可变。PEA issuer 从锁定的 pending
`PHASE_A_HARNESS` Execution/JobVersion/当前 revision/policy/namespace/PAG 快照插入；0024 的私有 issuer
只可在同一个事务内创建该 pending row、写 PEA 并取得同一 public global lock；PEA consumer
...
只有专用无登录 issuer 可执行 nonce/PAG 签发、撤回、PEA authorize 和 0024 原子 private-create 函数，专用无登录 consumer
...
不解密凭据也不启动 DataX。
它仍不是 protected issuer 的完整调用链：虽有 PEA ledger/issuer/consumer 和仅 issuer 可调用的原子数据库创建，
但没有私有 `rerun`、私有 API/Worker/Compose override、QH/PEA source、受保护 harness、四检查点
或真实跨进程运行路径；migration owner/数据库管理员也不是 HQA/RQA 信任根。标准启动与普通产品路径继续
production deny-all。

该 ledger 还必须与标准 backup/diagnostics 隔离：逻辑 dump 与诊断输出不得包含完整
`des_phase_a_qualification` schema（包括 0024 create checkpoints），restore 不得把它带到新实例。当前 restore 仍不能启动：
dump 会保留 current schema head `alembic_version=20260802_0024` 却排除该 schema。未来 protected restore bootstrap 才能创建
...
> **状态：0022/0023 的授权与 lock/fence 已有受限 PostgreSQL E2；0024 的 issuer-only atomic create/lifecycle 也已在 2026-08-02 取得受限 PostgreSQL E2（`32 passed`）。**
...
仍缺少私有 `rerun`、私有 Worker 领取/启动四检查点、Compose credential
provisioning、QH/PAG/PEA private source/override、受保护 harness、真实 DataX E3 和 QR。0024 的 private issuer
create→PEA→global `RESERVED` 仅是一个数据库原子入口，成功后仍 BLOCKED，不是普通/API/UI 创建或 runner 输入。每个未来
...
该表及 0024 的 private checkpoint 已位于完整私有 schema，因此标准 backup、diagnostics 和 restore 输入必须与 nonce/grant 一同
...
### 5.3.4 J0b.2 私有 create → PEA → global `TargetCopyLock` 原子基础

> **状态：`20260802_0024` 是 E1 开发基础；真实 PostgreSQL migration/atomic-create/lifecycle 验收已在 2026-08-02 取得受限 E2（`32 passed`）。**
> `phase-a-execution-lifecycle.v1.schema.json` 只描述已成功提交后的 protected private
> `LOCK_RESERVED` receipt，不是 HTTP/OpenAPI、Settings、Compose、Launcher 或普通用户契约，结论固定为
> `NOT_E3_OR_E4`。

0024 只新增 `datax_phase_a_issuer` 可执行的受保护数据库函数。它必须在**同一事务**内完成：

1. 创建 `authorization_mode=PHASE_A_HARNESS` 的 Execution；
2. 以既有 0022 issuer 逻辑写入不可变 PEA；
3. 在既有 **public** `target_copy_locks` 中为相同 `TargetNamespace` 写入 `RESERVED`。

在读取任一业务 current fact **之前**，函数先对
`public.system_control(singleton_id=1)` 执行 `SELECT ... FOR UPDATE`；该锁与 Launcher/Worker 的本地
stop/drain 更新采用相同线性化顺序，故 drain 与创建不能分别依据陈旧 admission 状态提交。PostgreSQL 的行锁
要求持有某列 `UPDATE` 权限，故仅无登录 ledger owner 获得窄至 `UPDATE(singleton_id)` 的权限；issuer 与
普通/runtime role 没有该直接权限，只能经固定 `SECURITY DEFINER` create 函数进入。
真实两连接 PostgreSQL E2 用 owner connection 持行锁、issuer connection 的 `pg_backend_pid()` 与
`pg_locks` 中的 `NOT granted` 等待证明这不是纯静态约束；owner 先写入 `draining=true` 后释放，issuer 必须
`P0001` 失败且 Execution/PEA/lock/checkpoint 零残留。该证据仍只覆盖数据库 E2，不启动 Worker/DataX。

授权、PAG/QH/current-fact、源静默/目标独占确认形状或有效窗口、PEA 唯一性、以及 shared partial-unique
目标锁中任一检查失败时，Execution、PEA、lock 与 private checkpoint 必须全部回滚；不得消耗 grant 后留下
可重绑、可自动重试的孤儿 PEA。成功端态只能是 `Execution=QUEUED`、
`queue_eligibility_state=BLOCKED`、`queue_block_reason=PHASE_A_PRIVATE_WORKER_NOT_IMPLEMENTED`、
`active_attempt_id=NULL`、`attempt_count=0`、`fence_epoch=0`，以及同一目标的
`TargetCopyLock=RESERVED`、无 attempt/fence。`RESERVED` 仅表示平台内目标锁预留，绝不表示可 claim、
Popen、DataX 或 retry。

`phase_a_execution_create_checkpoints` 的 `EXECUTION_CREATED/PEA_BOUND/LOCK_RESERVED` 是同一提交内的私有
append-only audit 行，不能成为独立提交、公开读取或重放 capability。外部 lifecycle Schema 只能表达最终
`LOCK_RESERVED` receipt，且不序列化 raw nonce、QH、凭据、DSN、命令、完整 DataX 配置、日志、结果或证据载荷。
0024 只校验并保存人工确认的输入形状与目标有效窗口；它不证明源端从未写入、不产生目标一致性快照，
也不替代未来 claim/Popen/oracle 前完整 PEA/current-fact/confirmation 重读。

标准 API、Worker、egress guard、runner、Settings、Compose、Launcher、普通维护/备份路径均没有该函数的
调用权或直接 DML 权；它们继续只处理 `STANDARD`。0024 不 provision private runner、不会 claim、不会创建
Attempt、不会解密凭据、不会启动 DataX 或写私有日志/审计/oracle；private runner、E3、E4 与公开能力仍为
`BLOCKED`；它们仍不是 E3/E4 或公开能力。

0024 downgrade 也是保护边界：检查前必须对 public `executions`、`execution_attempts`、`target_copy_locks` 以及
私有 grant、PEA、`phase_a_execution_create_checkpoints` 取得 `ACCESS EXCLUSIVE` 锁；任一 protected
`PHASE_A_HARNESS` Execution、PEA 或 checkpoint 行存在时即拒绝降级，不能为迁移回退删除仍可能代表私有
authority 的事实；该 fail-closed 行为已随 2026-08-02 的 PostgreSQL E2 验证。
当前没有 protected private disposition workflow：普通 API/Worker、backup、restore 或 migration rollback 都不能
处置这些对象；标准 backup 因任一 private Execution 而阻断，private backup/restore 仍为 `BLOCKED`。标准产品不
provision issuer login，也没有普通 API 调用该函数；protected disposition、runner 与 backup/restore 均未完成，
不得描述为可用运行路径。
...
| `reserved_at` | timestamptz | 是 | STANDARD API 创建 QUEUED Execution，或 0024 private issuer 原子创建完成时的预留时间 |
...
`RESERVED/ACTIVE/RECOVERY_REQUIRED`。STANDARD API 在创建 Execution、初始 Event/Audit、幂等
结果的同一事务创建 `RESERVED`；0024 private issuer 则只在 create→PEA→同一 public lock 的单事务
成功后创建它，且 private Execution 保持 BLOCKED。冲突时拒绝第二个运行意图。普通 Worker 领取时锁定该行，
...
- `20260802_0020` 为 PostgreSQL 专用迁移，在 closed-default
  ...
  角色凭据，且没有普通路径创建/rerun、Worker 四检查点或 DataX 启动调用链。`20260802_0023` 再创建
  ...
  private lock；没有 login provisioning 或产品接线。`20260802_0024` 只向 issuer 增加同一事务的 private
  Execution→PEA→public `RESERVED` 创建与 private checkpoint，任一失败完整回滚，成功后仍
  `QUEUED/BLOCKED/PHASE_A_PRIVATE_WORKER_NOT_IMPLEMENTED`；新增真实 PostgreSQL 验收已在 2026-08-02
  取得受限 E2（`32 passed`）。
  它不构成 protected harness、真实 DataX E3、Windows E4 或发布回滚证据。
- 任何支持 `20260802_0024` 及更高版本的标准 backup/diagnostics 都必须固定排除
...
23. Phase-A 私有 ledger 必须拒绝 nonce 重放、binding 修改、grant/PEA/checkpoint 删除或截断及非单调 grant
    lifecycle；标准运行角色和 issuer/consumer/runner 对四张私有表的实际直接读写/截断必须被 PostgreSQL 拒绝。
    ...
    `scripts/test-postgres-e2.sh` 退出 `0`、PostgreSQL pytest `32 passed`，已提供 0021/0022 role/function/
    ...
    dump→空库 `pg_restore` 探针的受限 E2。0024 的 issuer-only atomic create→PEA→public `RESERVED`、
    完整回滚、lifecycle function、角色拒绝和所有运行角色/issuer/consumer/runner 的 checkpoint direct-DML
    拒绝也已随 `32 passed` 验证；私有 Worker
