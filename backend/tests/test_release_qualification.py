...
RELEASE_QUALIFICATION_SCHEMA = (
    REPOSITORY_ROOT / "docs/contracts/release-qualification.v1.schema.json"
)
NOW = datetime(2026, 8, 2, 12, 0, tzinfo=UTC)
PAYLOAD_DOMAIN = b"DES-RELEASE-PAYLOAD-v1\n"
QUALIFICATION_DOMAIN = b"DES-HARNESS-QUALIFICATION-v1\n"
RELEASE_QUALIFICATION_DOMAIN = b"DES-RELEASE-QUALIFICATION-v1\n"
...
def _payload(
    payload_signer: Ed25519PrivateKey,
    *,
    release_signer: Ed25519PrivateKey | None = None,
) -> dict[str, object]:
    public_key = payload_signer.public_key().public_bytes(
        serialization.Encoding.Raw,
        serialization.PublicFormat.Raw,
    )
    release_signer = release_signer or Ed25519PrivateKey.generate()
    rqa_public_key = release_signer.public_key().public_bytes(
        serialization.Encoding.Raw,
        serialization.PublicFormat.Raw,
    )
    commit = "a" * 40
...
def _release_binding(payload: dict[str, object]) -> dict[str, object]:
    worker_runtime = payload["worker_runtime"]
    artifacts = payload["artifacts"]
    assert isinstance(worker_runtime, dict)
    assert isinstance(artifacts, dict)
    plugins = worker_runtime["plugins"]
    assert isinstance(plugins, list)
    return {
        "payload_root_sha256": payload["payload_root_sha256"],
        "payload_binding_sha256": hashlib.sha256(_canonical(_binding(payload))).hexdigest(),
        "commit_sha": payload["identity"]["commit_sha"],  # type: ignore[index]
        "release_candidate": payload["identity"]["release_candidate"],  # type: ignore[index]
        "build_identity_sha256": artifacts["build_identity"]["sha256"],  # type: ignore[index]
        "image_lock_sha256": artifacts["image_lock"]["sha256"],  # type: ignore[index]
        "worker_image_digest": f"sha256:{_hash('worker')}",
        "runtime_tree_sha256": worker_runtime["runtime_tree_sha256"],
        "plugins": [
            {
                "name": plugin["name"],
                "jar_sha256": plugin["jar"]["sha256"],
                "parameter_boundary_contract_version": plugin[
...
def _dependency_license_review(payload: dict[str, object]) -> dict[str, object]:
    artifacts = payload["artifacts"]
    assert isinstance(artifacts, dict)
    image_sboms = artifacts["image_sboms"]
    assert isinstance(image_sboms, list)
    return {
        "source_sbom_sha256": artifacts["source_sbom"]["sha256"],  # type: ignore[index]
        "image_sboms": [
            {"role": item["role"], "sha256": item["artifact"]["sha256"]}
            for item in image_sboms
        ],
        "license_inventory_sha256": artifacts["license_inventory"]["sha256"],  # type: ignore[index]
...
def _evidence(label: str) -> dict[str, str]:
    return {
        "evidence_id": f"evidence-{label}-0001",
        "description": f"Signed qualification evidence for {label}",
        "sha256": _hash(f"evidence-{label}"),
    }


def _release_qualification_unsigned(payload: dict[str, object]) -> dict[str, object]:
    release_binding = _release_binding(payload)
    plugins = release_binding["plugins"]
    assert isinstance(plugins, list)
    mysql_reader = plugins[0]
    mysql_writer = plugins[1]
    return {
        "schema_version": "1.0",
        "artifact_kind": "RELEASE_QUALIFICATION",
        "authorization_id": "qr-authorization-0001",
        "purpose": "PRIVATE_PHASE_B_QUALIFICATION",
        "issuer_key_id": "rqa-key-0001",
        "issued_at": "2026-08-02T11:00:00Z",
        "not_before": "2026-08-02T11:00:00Z",
        "valid_until": "2026-08-09T11:00:00Z",
        "release_binding": release_binding,
        "dependency_license_review": _dependency_license_review(payload),
        "qualification_rows": [
            {
                "reader": {
                    "name": mysql_reader["name"],
                    "jar_sha256": mysql_reader["jar_sha256"],
                    "parameter_boundary_contract_version": mysql_reader[
                        "parameter_boundary_contract_version"
                    ],
                },
                "writer": {
                    "name": mysql_writer["name"],
                    "jar_sha256": mysql_writer["jar_sha256"],
                    "parameter_boundary_contract_version": mysql_writer[
                        "parameter_boundary_contract_version"
                    ],
                },
                "evidence": {
                    "phase_a_e3_evidence": _evidence("e3"),
                    "phase_a_private_windows_evidence": _evidence("private-windows"),
                    "payload_qualification_evidence": _evidence("payload"),
                },
            }
        ],
    }
...
def test_qr_verifier_has_no_standard_product_wiring() -> None:
    standard_paths = [
        REPOSITORY_ROOT / "backend/src/datax_studio/settings.py",
        REPOSITORY_ROOT / "backend/src/datax_studio/plugin_certification.py",
        REPOSITORY_ROOT / "backend/src/datax_studio/datax_runner.py",
        REPOSITORY_ROOT / "deploy/windows/compose.yaml",
        *sorted((REPOSITORY_ROOT / "backend/src/datax_studio/api").rglob("*.py")),
        *sorted((REPOSITORY_ROOT / "backend/src/datax_studio/worker").rglob("*.py")),
    ]

    for path in standard_paths:
        contents = path.read_text(encoding="utf-8")
        assert "release_qualification" not in contents, path
        assert "ExpectedPluginPair" not in contents, path
        assert "verify_release_qualification" not in contents, path
...
def test_canonical_qr_verifies_only_against_the_rqa_key_pinned_by_payload() -> None:
    payload_signer = Ed25519PrivateKey.generate()
    release_signer = Ed25519PrivateKey.generate()
    payload = _payload(payload_signer, release_signer=release_signer)
    unsigned = _release_qualification_unsigned(payload)
    qualification = _signed_qualification(
        unsigned,
        release_signer,
        domain=RELEASE_QUALIFICATION_DOMAIN,
    )

    Draft202012Validator(
        json.loads(RELEASE_QUALIFICATION_SCHEMA.read_text(encoding="utf-8")),
        format_checker=FormatChecker(),
    ).validate(qualification)

    verified = verify_release_qualification(
        _canonical(qualification),
        release_payload=_parsed_payload(payload),
        expected_pair=ExpectedPluginPair("mysqlreader", "mysqlwriter"),
        now=NOW,
    )

    assert verified.authorization_id == "qr-authorization-0001"
    assert verified.payload_root_sha256 == payload["payload_root_sha256"]
    assert verified.issuer_key_id == "rqa-key-0001"
    assert (verified.reader_name, verified.writer_name) == ("mysqlreader", "mysqlwriter")
    assert not hasattr(verified, "ordinary_user_executable")
    assert not hasattr(verified, "windows_e4_certified")


def test_qr_rejects_tamper_self_key_wrong_pair_and_expired_authorization() -> None:
    payload_signer = Ed25519PrivateKey.generate()
    release_signer = Ed25519PrivateKey.generate()
    payload = _payload(payload_signer, release_signer=release_signer)
    unsigned = _release_qualification_unsigned(payload)
    ...
    tampered["release_binding"]["runtime_tree_sha256"] = _hash("forged-runtime")  # type: ignore[index]
    _assert_code(
        lambda: verify_release_qualification(
            _canonical(
                _signed_qualification(
                    tampered,
                    release_signer,
                    domain=RELEASE_QUALIFICATION_DOMAIN,
                )
    ...
    _assert_code(
        lambda: verify_release_qualification(
            ...
            release_payload=_parsed_payload(payload),
            expected_pair=ExpectedPluginPair("postgresqlreader", "postgresqlwriter"),
            now=NOW,
        ),
        "QR_PLUGIN_PAIR_NOT_QUALIFIED",
    )
    expired = copy.deepcopy(unsigned)
    expired["issued_at"] = "2026-08-01T11:00:00Z"
    expired["not_before"] = "2026-08-01T11:00:00Z"
    ...
    _assert_code(
        ...
        "QR_NOT_CURRENTLY_VALID",
    )


def test_qr_rejects_wrong_rqa_key_domain_or_oversized_validity_window() -> None:
    payload_signer = Ed25519PrivateKey.generate()
    release_signer = Ed25519PrivateKey.generate()
    payload = _payload(payload_signer, release_signer=release_signer)
    unsigned = _release_qualification_unsigned(payload)
    ...
    _assert_code(
        lambda: verify_release_qualification(
            _canonical(_signed_qualification(unsigned, release_signer, domain=b"wrong-domain\n")),
            release_payload=_parsed_payload(payload),
            expected_pair=ExpectedPluginPair("mysqlreader", "mysqlwriter"),
            now=NOW,
        ),
    ...
    too_long = copy.deepcopy(unsigned)
    too_long["valid_until"] = "2026-09-02T11:00:01Z"
    _assert_code(
        lambda: verify_release_qualification(
            _canonical(
                _signed_qualification(
        ...
        "QR_VALIDITY_WINDOW_TOO_LONG",
    )


def test_qr_rejects_unknown_key_purpose_noncanonical_json_and_pair_binding_drift() -> None:
    payload_signer = Ed25519PrivateKey.generate()
    release_signer = Ed25519PrivateKey.generate()
    payload = _payload(payload_signer, release_signer=release_signer)
    unsigned = _release_qualification_unsigned(payload)

    unknown_key = copy.deepcopy(unsigned)
    unknown_key["issuer_key_id"] = "rqa-key-unknown"
    _assert_code(
        lambda: verify_release_qualification(
            _canonical(
                _signed_qualification(
        ...
        "QR_ISSUER_KEY_UNKNOWN",
    )
    wrong_purpose = copy.deepcopy(unsigned)
    wrong_purpose["purpose"] = "QUALIFICATION_HARNESS"
    _assert_code(
        lambda: verify_release_qualification(
            _canonical(
                _signed_qualification(
    ...
    _assert_code(
        ...
        "RELEASE_QUALIFICATION_CANONICAL_JSON_REQUIRED",
    )
    pair_binding_drift = copy.deepcopy(unsigned)
    pair_binding_drift["qualification_rows"][0]["reader"]["jar_sha256"] = _hash(  # type: ignore[index]
        "forged-reader-jar"
    )
    _assert_code(
        lambda: verify_release_qualification(
...
def test_release_payload_and_qh_contracts_match_the_strict_parser_boundaries() -> None:
    ...
    shared_authority_key = copy.deepcopy(payload)
    shared_authority_key["rqa_keyring"]["keys"][0]["public_key_base64"] = (  # type: ignore[index]
        shared_authority_key["hqa_keyring"]["keys"][0]["public_key_base64"]  # type: ignore[index]
    )
    _refresh_payload_root(shared_authority_key)
    assert _schema_accepts(PAYLOAD_SCHEMA, shared_authority_key)
    _assert_code(
        lambda: _parsed_payload(shared_authority_key),
        "RELEASE_PAYLOAD_KEYRING_SEPARATION_INVALID",
    )
